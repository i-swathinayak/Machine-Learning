...
class KNN:
    ...
    def predict(self, features):
        ...
        for point in features:
            if sum(self.get_k_neighbors(point)) >= self.k / 2:
                predict_labels[i] = 1
    ...
    def get_k_neighbors(self, point):
        ...
        train_distances = [0.0 for i in range(self.features.shape[0])]
        train_labels = self.labels.tolist()
        for i in range(0,self.features.shape[0]):
            train_distances[i] = self.distance_function(self.features[i],point) 
        return [a for _, a in sorted(zip(train_distances,train_labels))][0:self.k]
